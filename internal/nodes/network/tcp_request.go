...
func (n *TCPRequestNode) Init() error {
	...
	n.hostTemplate = strings.TrimSpace(nodes.StringVal(props, "host", ""))
	if n.hostTemplate != "" {
	...
	n.portTemplate = strings.TrimSpace(nodes.StringVal(props, "port", ""))
	if n.portTemplate != "" {
	...
	n.terminator = strings.ToLower(nodes.StringVal(props, "terminator", tcpReqTermTime))
	switch n.terminator {
	...
	if n.terminator == tcpReqTermDelimiter {
		raw := nodes.StringVal(props, "delimiter", "\\n")
	...
	if n.terminator == tcpReqTermLength {
		n.responseLength = nodes.IntVal(props, "responseLength", 0)
	...
	if raw := nodes.StringVal(props, "appendDelimiter", ""); raw != "" {
		bs, err := ParseDelimiter(raw)
		if err != nil {
	...
	n.responseEnc = strings.ToLower(nodes.StringVal(props, "responseEncoding", tcpEncBuffer))
	...
	timeoutMs := nodes.IntVal(props, "responseTimeout", int(defaultTcpResponseTimeout/time.Millisecond))
	if timeoutMs <= 0 {
		timeoutMs = int(defaultTcpResponseTimeout / time.Millisecond)
	}
	n.responseTimeout = time.Duration(timeoutMs) * time.Millisecond

	dialSec := nodes.IntVal(props, "dialTimeout", int(defaultTcpDialTimeout/time.Second))
	if dialSec <= 0 {
