...
func (n *ModbusReadNode) Init() error {
	...
	n.quantity = nodes.IntVal(props, "quantity", 1)
	if n.quantity < 1 {
		n.quantity = 1
	}
...
func (n *ModbusReadNode) effectiveParams(msg *flow.Message) readParams {
	...
	if v, ok := nodes.ReadPositiveInt(msg.Get("fc")); ok && v >= 1 && v <= 4 {
		p.fc = v
	}
	...
	if v, ok := nodes.ReadPositiveInt(msg.Get("quantity")); ok && v >= 1 {
		p.quantity = v
	}
...
func (n *ModbusReadNode) doRead(p readParams) (*flow.Message, error) {
	...
	scaled := nodes.ApplyScale(decoded, n.scale, n.offset)
