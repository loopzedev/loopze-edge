...
func (n *TCPInNode) Init() error {
	...
	n.mode = strings.ToLower(nodes.StringVal(props, "mode", tcpInModeServer))
	if n.mode != tcpInModeServer && n.mode != tcpInModeClient {
		return fmt.Errorf("tcp-in %s: invalid mode %q", n.cfg.ID, n.mode)
	}

	n.host = strings.TrimSpace(nodes.StringVal(props, "host", ""))
	...
	n.maxFrameBytes = nodes.IntVal(props, "maxFrameBytes", defaultTcpInMaxFrame)
	...
	kaSec := nodes.IntVal(props, "keepAliveInterval", int(defaultTcpInKeepAliveInterval/time.Second))
	if kaSec <= 0 {
	...
	if n.mode == tcpInModeServer {
		...
		n.maxConnections = nodes.IntVal(props, "maxConnections", 0)
	...
	if n.mode == tcpInModeClient {
		...
		ms := nodes.IntVal(props, "reconnectInitialDelay", int(defaultTcpInReconnectInitialDelay/time.Millisecond))
		if ms <= 0 {
			ms = int(defaultTcpInReconnectInitialDelay / time.Millisecond)
		}
		n.reconnectInitial = time.Duration(ms) * time.Millisecond
		ms = nodes.IntVal(props, "reconnectMaxDelay", int(defaultTcpInReconnectMaxDelay/time.Millisecond))
		if ms <= 0 {
			ms = int(defaultTcpInReconnectMaxDelay / time.Millisecond)
		}
		n.reconnectMax = time.Duration(ms) * time.Millisecond
		ds := nodes.IntVal(props, "dialTimeout", int(defaultTcpInDialTimeout/time.Second))
		if ds <= 0 {
			ds = int(defaultTcpInDialTimeout / time.Second)
		}
		n.dialTimeout = time.Duration(ds) * time.Second

		tlsCfg, err := nodes.ParseTLSBlock(props, n.cfg.ID, n.certStore)
		if err != nil {
...
func (n *TCPInNode) parseFraming(props map[string]any) error {
	mode := strings.ToLower(nodes.StringVal(props, "framing", FramingStream))
	switch mode {
	...
	if mode == FramingDelimiter {
		bs, err := ParseDelimiter(nodes.StringVal(props, "delimiter", `\n`))
		if err != nil {
			return fmt.Errorf("tcp-in %s: %w", n.cfg.ID, err)
		}
	...
	if mode == FramingLengthPrefix {
		...
		bytesN := nodes.IntVal(raw, "bytes", 4)
		endian, err := ParseEndianness(nodes.StringVal(raw, "endianness", "big"))
		if err != nil {
			return fmt.Errorf("tcp-in %s: %w", n.cfg.ID, err)
		}
	...
	if mode == FramingFixedLength {
		size := nodes.IntVal(props, "fixedLength", 0)
		if size <= 0 {
