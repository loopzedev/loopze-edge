...
func (n *ModbusParserNode) Init() error {
	...
	n.parseFrom = nodes.StringVal(props, "parseFrom", "bytes")
	n.encodeFrom = nodes.StringVal(props, "encodeFrom", "payload")
	n.byteOrder = parseByteOrder(nodes.StringVal(props, "byteOrder", ""))
	n.wordOrder = parseWordOrder(nodes.StringVal(props, "wordOrder", ""))
...
func parseLayoutField(m map[string]any) (modbusField, error) {
	f := modbusField{
		Offset: nodes.IntVal(m, "offset", -1),
		Name:   nodes.StringVal(m, "name", ""),
		Type:   nodes.StringVal(m, "type", ""),
		Length: nodes.IntVal(m, "length", 0),
		Bit:    nodes.IntVal(m, "bit", -1),
...
func (n *ModbusParserNode) parse(regs []uint16) (map[string]any, error) {
	...
	for _, f := range n.layout {
		...
		if f.Type != "string" && f.Type != "raw" && f.Type != "bool" {
			v = nodes.ApplyScale(v, f.Scale, f.OffsetValue)
		}
		out[f.Name] = v
	}
	return out, nil
