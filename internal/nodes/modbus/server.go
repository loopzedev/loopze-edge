...
func NewModbusServer(cfg flow.ConfigNode) (flow.ConfigInstance, error) {
	...
	timeoutMs := nodes.IntVal(props, "timeout", 1000)
	timeout := time.Duration(timeoutMs) * time.Millisecond

	idleTimeoutSec := nodes.IntVal(props, "idleTimeout", 60)
	idleTimeout := time.Duration(idleTimeoutSec) * time.Second
	if idleTimeoutSec == 0 {
		idleTimeout = 0
	}

	defaultUnit := byte(nodes.IntVal(props, "defaultUnitId", 1))
	...
	case "tcp":
		...
		port := nodes.IntVal(props, "port", 502)
	...
	case "rtu":
		...
		baud := nodes.IntVal(props, "baudRate", 9600)
		dataBits := nodes.IntVal(props, "dataBits", 8)
		stopBits := nodes.IntVal(props, "stopBits", 1)
