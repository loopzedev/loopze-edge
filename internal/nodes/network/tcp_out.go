...
func (n *TCPOutNode) Init() error {
	...
	n.mode = strings.ToLower(nodes.StringVal(props, "mode", tcpOutModeReply))
	switch n.mode {
	...
	if n.mode == tcpOutModeServerBroadcast {
		n.targetTcpIn = strings.TrimSpace(nodes.StringVal(props, "targetTcpIn", ""))
		if n.targetTcpIn == "" {
	...
	dialSec := nodes.IntVal(props, "dialTimeout", int(defaultTcpOutDialTimeout/time.Second))
	if dialSec <= 0 {
		dialSec = int(defaultTcpOutDialTimeout / time.Second)
	}
	n.dialTimeout = time.Duration(dialSec) * time.Second

	writeSec := nodes.IntVal(props, "writeTimeout", int(defaultTcpOutWriteTimeout/time.Second))
	if writeSec <= 0 {
		writeSec = int(defaultTcpOutWriteTimeout / time.Second)
	}
	n.writeTimeout = time.Duration(writeSec) * time.Second

	n.outboundQueueSz = nodes.IntVal(props, "outboundQueueSize", defaultTcpOutQueueSize)
