...
func startDelay(t *testing.T, properties map[string]any) (*nodestest.Collector, flow.NodeInstance) {
	t.Helper()
...
func TestRateMode_QueueOverflow(t *testing.T) {
	...
	if c.Count() != 3 {
		t.Fatalf("expected 3 messages drained from queue (size=3), got %d", c.Count())
	}
