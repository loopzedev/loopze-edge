...
func (n *InjectNode) Init() error {
	...
	n.mode = nodes.StringVal(cfgProps, "mode", "interval")
	...
	if ok && len(rawProps) > 0 {
		for _, raw := range rawProps {
			...
			n.props = append(n.props, InjectProp{
				Property: nodes.StringVal(propMap, "p", "payload"),
				VType:    nodes.StringVal(propMap, "vt", "str"),
				Value:    nodes.StringVal(propMap, "v", ""),
				Storage:  nodes.StringVal(propMap, "vs", "memory"),
			})
		}
	}
...
func (n *InjectNode) valueContext() nodes.ValueContext {
...
func (n *InjectNode) emit() {
	...
	for _, prop := range n.props {
		val, err := nodes.ResolveValue(prop.VType, prop.Value, prop.Storage, nil, ctx)
		if err != nil {
