...
// the Catch Node itself and other Catch Nodes (errors raised by Catch
// Nodes are filtered at the engine level, so picking them is pointless).
const candidates = computed(() => {
  const selfId = flowStore.selectedNode?.id
  return flowStore.activeNodes
    .filter((n) => n.id !== selfId && n.type !== 'catch')
    .map((n) => ({
      id: n.id,
...
<template>
  <div class="flex flex-col gap-3">
    ...
    <FormField v-if="scope === 'selected'" label="Catch errors from">
      <div
    ...
    <p class="text-[10px] text-terminal-text-dim leading-relaxed">
      Emits one message per error raised by a node within the selected scope.
      Catch Nodes themselves are excluded — they cannot trigger each other.
      The outgoing message carries
      <code class="text-terminal-text">msg._error</code> with
      <code class="text-terminal-text">message</code> and a
      <code class="text-terminal-text">source</code> object; the original
      message payload is preserved.
      Catch is meant for logging and notifications — it is not a retry
      mechanism (failures along a catch branch are intentionally not
      re-caught).
    </p>
  </div>
</template>
