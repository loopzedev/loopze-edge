...
func waitForCount(t *testing.T, col *nodestest.Collector, want int, timeout time.Duration) {
	t.Helper()
...
func TestTcpInServerAcceptAndFrameDelimiter(t *testing.T) {
	...
	if got := h.col.At(0).Payload(); got != "hello" {
		t.Errorf("frame[0] = %q", got)
	}
	if got := h.col.At(1).Payload(); got != "world" {
		t.Errorf("frame[1] = %q", got)
	}
	// session handle present and resolvable.
	handle, _ := h.col.At(0).Get("session").(*flow.SessionHandle)
	if handle == nil {
...
func TestTcpInServerFixedLength(t *testing.T) {
	...
	waitForCount(t, h.col, 2, time.Second)
	if got := h.col.At(0).Payload(); got != "aaaa" {
		t.Errorf("frame[0] = %q", got)
	}
	if got := h.col.At(1).Payload(); got != "bbbb" {
		t.Errorf("frame[1] = %q", got)
	}
}
...
func TestTcpInClientConnects(t *testing.T) {
	...
	if got := h.col.At(0).Payload(); got != "greetings" {
		t.Errorf("payload = %q", got)
	}
}
...
func TestTcpInServerStreamFraming(t *testing.T) {
	...
	waitForCount(t, h.col, 1, time.Second)
	if got := h.col.At(0).Payload(); got != "chunk1" {
		t.Errorf("payload = %q", got)
	}
}
