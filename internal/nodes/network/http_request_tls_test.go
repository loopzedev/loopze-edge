...
func TestHTTPRequest_NewTLSBlockWithCABundleRef(t *testing.T) {
	store := nodestest.NewCertStore(t)
	now := time.Now()
	caPEM, _ := nodestest.GenerateTLSPair(t, "internal-ca", now.Add(-time.Hour), now.Add(time.Hour))
...
func TestHTTPRequest_NewTLSBlockWinsOverLegacyBoolean(t *testing.T) {
	store := nodestest.NewCertStore(t)
	now := time.Now()
	caPEM, _ := nodestest.GenerateTLSPair(t, "trusted", now.Add(-time.Hour), now.Add(time.Hour))
	if _, err := store.Store(credentials.CertEntry{
...
func TestHTTPRequest_ConflictingInlineAndRef_Errors(t *testing.T) {
	store := nodestest.NewCertStore(t)
	now := time.Now()
	caPEM, _ := nodestest.GenerateTLSPair(t, "ca", now.Add(-time.Hour), now.Add(time.Hour))
