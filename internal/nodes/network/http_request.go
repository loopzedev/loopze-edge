...
func (n *HTTPRequestNode) Init() error {
	...
	n.urlTemplate = strings.TrimSpace(nodes.StringVal(props, "url", ""))
	if n.urlTemplate != "" {
	...
	n.responseFormat = strings.ToLower(nodes.StringVal(props, "responseFormat", respFormatAuto))
	...
	n.bodyEncoding = strings.ToLower(nodes.StringVal(props, "bodyEncoding", bodyEncodingAuto))
	...
	if raw, ok := props["auth"].(map[string]any); ok {
		n.auth = &requestAuthCfg{
			kind:     nodes.StringVal(raw, "type", "none"),
			username: nodes.StringVal(raw, "username", ""),
			password: nodes.StringVal(raw, "password", ""),
			token:    nodes.StringVal(raw, "token", ""),
		}
	}

	timeoutSec := nodes.IntVal(props, "timeout", int(defaultRequestTimeout/time.Second))
	if timeoutSec <= 0 {
	...
	n.errorMode = strings.ToLower(nodes.StringVal(props, "errorMode", errModePassthrough))
