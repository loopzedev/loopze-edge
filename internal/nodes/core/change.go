...
func (n *ChangeNode) Init() error {
	...
	for i, raw := range rawRules {
		...
		rule := Rule{
			Type:     nodes.StringVal(ruleMap, "t", "set"),
			Property: nodes.StringVal(ruleMap, "p", "payload"),
			PropType:    nodes.StringVal(ruleMap, "pt", "msg"),
			PropStorage: nodes.StringVal(ruleMap, "ps", "memory"),
			To:          nodes.StringVal(ruleMap, "to", ""),
			ToType:      nodes.StringVal(ruleMap, "tot", "str"),
			ToStorage:   nodes.StringVal(ruleMap, "tos", "memory"),
			From:        nodes.StringVal(ruleMap, "from", ""),
			FromType:    nodes.StringVal(ruleMap, "fromt", "str"),
			FromStorage: nodes.StringVal(ruleMap, "froms", "memory"),
		}
...
// valueContext builds a nodes.ValueContext from the node's context stores.
func (n *ChangeNode) valueContext() nodes.ValueContext {
...
func (n *ChangeNode) pickContextStore(scope, storage string) flow.ContextStore {
	return nodes.PickContextStore(n.valueContext(), scope, storage)
}
